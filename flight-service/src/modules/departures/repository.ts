...
export const departuresRepository = {
  ...
  async findByFlightNumber(flightNumber: string) {
    const departures = await db
      ...
      .where(
        and(
          eq(departuresTable.departed, false),
          eq(departuresTable.flightNumber, flightNumber),
  ...
  async update(id: string, data: any) {
    const result = await db
      ...
      .set(data)
      .where(and(eq(departuresTable.id, id), isNull(departuresTable.deletedAt)))
      .returning();
    return result;
  },

  async delete(id: string) {
    const deletedDeparture = await db
      .update(departuresTable)
      .set({
        deletedAt: new Date(),
      })
      .where(
        and(eq(departuresTable.id, id), isNull(departuresTable.deletedAt)),
      );
