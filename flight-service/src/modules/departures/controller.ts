...
export const getDepartures = async (
  request: FastifyRequest<{ Querystring: FindDepartureInput }>,
  _reply: FastifyReply,
) => {
  if (request.query.planeReg) {
    return departuresService.findByPlane(request.query.planeReg);
  }
  if (request.query.flightNumber) {
    return departuresService.findByFlightNumber(request.query.flightNumber);
  }
  return departuresService.getAll();
};

export const createDeparture = async (
  request: FastifyRequest<{ Body: CreateDepartureInput }>,
  reply: FastifyReply,
) => {
...
export const editDeparture = async (
  request: FastifyRequest<{ Body: EditDepartureInput; Params: IdParams }>,
  reply: FastifyReply,
) => {
  const departure = await departuresService.edit(
    request.params.id,
    request.body,
  );
...
export const deleteDeparture = async (
